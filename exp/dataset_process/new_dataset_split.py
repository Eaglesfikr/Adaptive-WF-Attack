# python exp/dataset_process/new_dataset_split.py --infile day0
import numpy as np
import os
import random
import argparse
...
parser.add_argument("--infile", type=str, required=True, default="data.npz", help="input file")
parser.add_argument("--use_stratify", type=str, default="True", help="Whether to use stratify")

# Parse arguments
args = parser.parse_args()
infile = os.path.join("datasets/NetCLR+", f"{args.infile}.npz")
...
np.savez(os.path.join(dataset_path, f"{args.infile}_train.npz"), X = X_train, y = y_train)
np.savez(os.path.join(dataset_path, f"{args.infile}_valid.npz"), X = X_valid, y = y_valid)
np.savez(os.path.join(dataset_path, f"{args.infile}_test.npz"), X = X_test, y = y_test)
