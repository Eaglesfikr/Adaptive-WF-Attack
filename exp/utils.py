...
def cal_GMM(train_iter, cur_model, seq_len, device):
    ...
    with torch.no_grad():
        ...
        for index, cur_data in enumerate(train_iter):
            ...
            all_soft_labels.append(outs.softmax(1).cpu().numpy())

    all_entropy = np.concatenate(all_entropy).flatten()
    all_preds = np.concatenate(all_preds).flatten()
    all_soft_labels = np.concatenate(all_soft_labels,axis=0)
    # 归一化
    ...
    all_soft_labels = torch.tensor(all_soft_labels, dtype=torch.float32)
    ...
    return prob, all_preds, all_soft_labels
