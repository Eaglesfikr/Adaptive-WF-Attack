...
train_iter = data_processor.load_iter(test_X, random_y, args.batch_size, False, args.num_workers)
...
ares_optimizer = torch.optim.SGD(ares_model.parameters(), lr=1e-5, weight_decay=0.01)
...
for epoch in range(5):
    clean_probs_ares, preds_ares, soft_labels_ares = cal_GMM(train_iter, ares_model, 10000, device)
    pseudo_loader_ares, unlabeled_loader_ares = build_loader(clean_probs_ares, test_X, preds_ares, soft_labels_ares, test_y, epoch)
    for _ in range(5):
